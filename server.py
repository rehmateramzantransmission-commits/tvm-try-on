...
class TryOnEngine:
    ...
    def process_frame(self, frame_b64: str, product_id: str, prompt: str = "", environment: str = "none") -> tuple[Optional[str], float]:
        ...
        if fal_key:
            try:
                import fal_client
                prompt_text = prompt or f"((masterpiece, highly detailed, photorealistic 2.5d video frame)):1.3 of person wearing luxurious tailor-fitted {product_id} suit outfit, sharp suit fabric textures, satin lapels, bow tie, perfect body alignment, seamless apparel transformation"
        ...
        if HAS_TORCH_CUDA and torch_lcm_pipe is not None:
            try:
                ...
                if frame_cv is not None:
                    ...
                    # --- 100% Face & Head Protection Compositing ---
                    # Preserve original camera face pixels above neck line to eliminate facial morphing
                    h_c, w_c = frame_cv.shape[:2]
                    face_protect_y = int(h_c * 0.40)  # Top 40% (face & head) remains 100% raw original camera feed
                    out_bgr[:face_protect_y, :, :] = frame_cv[:face_protect_y, :, :]

                    jpeg_bytes = encode_jpeg(out_bgr, quality=85)
        ...
        try:
            ...
            frame = decode_jpeg(img_data)

            if frame is None:
                raise ValueError("Could not decode image")

            # Standardize rendering resolution to 512x512 for ultra-low latency
            ...
            if has_pose and has_garment:
                ...
                if shoulders_visible:
                    ...
                    if warped.shape[2] == 4:
                        ...
                        # --- Face & Chin Protection Masking ---
                        # Zero out alpha mask above chin line so garment never bleeds onto face
                        chin_cutoff_y = max(0, int((top_y - 0.02) * h))
                        alpha[:chin_cutoff_y, :] = 0.0
            ...
            if 'frame' in locals() and frame is not None:
                jpeg_bytes = encode_jpeg(frame, quality=80)
                out_b64 = base64.b64encode(jpeg_bytes).decode('utf-8')
                return out_b64, (time.time() - start_time) * 1000
            elif 'raw_frame' in locals() and raw_frame is not None:
                jpeg_bytes = encode_jpeg(raw_frame, quality=80)
                out_b64 = base64.b64encode(jpeg_bytes).decode('utf-8')
                return out_b64, (time.time() - start_time) * 1000
            return None, 0.0
